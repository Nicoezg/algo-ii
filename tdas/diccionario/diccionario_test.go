...
func TestDiccionarioVacio(t *testing.T) {
	t.Log("Comprueba que Diccionario vacio no tiene SET_STRING_CLAVES")
...
func TestDiccionarioGuardar(t *testing.T) {
	...
	valor2 := "guau"
	valor3 := "moo"
	SET_STRING_CLAVES := []string{clave1, clave2, clave3}
	SET_STRING_DATOS := []string{valor1, valor2, valor3}

	dic := TDADiccionario.CrearHash[string, string]()
	require.False(t, dic.Pertenece(SET_STRING_CLAVES[0]))
	require.False(t, dic.Pertenece(SET_STRING_CLAVES[0]))
	...
	require.False(t, dic.Pertenece(SET_STRING_CLAVES[2]))
	dic.Guardar(SET_STRING_CLAVES[1], SET_STRING_DATOS[1])
	require.True(t, dic.Pertenece(SET_STRING_CLAVES[0]))
	require.True(t, dic.Pertenece(SET_STRING_CLAVES[1]))
	require.EqualValues(t, 2, dic.Cantidad())
	require.EqualValues(t, SET_STRING_DATOS[0], dic.Obtener(SET_STRING_CLAVES[0]))
	require.EqualValues(t, SET_STRING_DATOS[1], dic.Obtener(SET_STRING_CLAVES[1]))

	require.False(t, dic.Pertenece(SET_STRING_CLAVES[2]))
	dic.Guardar(SET_STRING_CLAVES[2], SET_STRING_DATOS[2])
	require.True(t, dic.Pertenece(SET_STRING_CLAVES[0]))
	require.True(t, dic.Pertenece(SET_STRING_CLAVES[1]))
	require.True(t, dic.Pertenece(SET_STRING_CLAVES[2]))
	require.EqualValues(t, 3, dic.Cantidad())
	require.EqualValues(t, SET_STRING_DATOS[0], dic.Obtener(SET_STRING_CLAVES[0]))
	require.EqualValues(t, SET_STRING_DATOS[1], dic.Obtener(SET_STRING_CLAVES[1]))
	require.EqualValues(t, SET_STRING_DATOS[2], dic.Obtener(SET_STRING_CLAVES[2]))
}

func TestReemplazoDato(t *testing.T) {
	t.Log("Guarda un par de SET_STRING_CLAVES, y luego vuelve a guardar, buscando que el dato se haya reemplazado")
	clave := "Gato"
	clave2 := "Perro"
	dic := TDADiccionario.CrearHash[string, string]()
...
func TestReemplazoDatoHopscotch(t *testing.T) {
	t.Log("Guarda bastantes SET_STRING_CLAVES, y luego reemplaza sus datos. Luego valida que todos los datos sean " +
...
func TestDiccionarioBorrar(t *testing.T) {
	...
	require.True(t, dic.Pertenece(SET_STRING_CLAVES[2]))
	require.EqualValues(t, SET_STRING_DATOS[2], dic.Borrar(SET_STRING_CLAVES[2]))
	require.PanicsWithValue(t, "La clave no pertenece al diccionario", func() { dic.Borrar(SET_STRING_CLAVES[2]) })
	require.EqualValues(t, 2, dic.Cantidad())
	require.False(t, dic.Pertenece(SET_STRING_CLAVES[2]))

	require.True(t, dic.Pertenece(SET_STRING_CLAVES[0]))
	require.EqualValues(t, SET_STRING_DATOS[0], dic.Borrar(SET_STRING_CLAVES[0]))
	require.PanicsWithValue(t, "La clave no pertenece al diccionario", func() { dic.Borrar(SET_STRING_CLAVES[0]) })
	require.EqualValues(t, 1, dic.Cantidad())
	require.False(t, dic.Pertenece(SET_STRING_CLAVES[0]))
	require.PanicsWithValue(t, "La clave no pertenece al diccionario", func() { dic.Obtener(SET_STRING_CLAVES[0]) })

	require.True(t, dic.Pertenece(SET_STRING_CLAVES[1]))
	require.EqualValues(t, SET_STRING_DATOS[1], dic.Borrar(SET_STRING_CLAVES[1]))
	require.PanicsWithValue(t, "La clave no pertenece al diccionario", func() { dic.Borrar(SET_STRING_CLAVES[1]) })
	require.EqualValues(t, 0, dic.Cantidad())
	require.False(t, dic.Pertenece(SET_STRING_CLAVES[1]))
	require.PanicsWithValue(t, "La clave no pertenece al diccionario", func() { dic.Obtener(SET_STRING_CLAVES[1]) })
}
...
func TestConSET_STRING_CLAVESNumericas(t *testing.T) {
...
func TestConSET_STRING_CLAVESStructs(t *testing.T) {
...
func TestCadenaLargaParticular(t *testing.T) {
	...
	SET_STRING_CLAVES := make([]string, 10)
	cadena := "%d~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" +
		"~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
	dic := TDADiccionario.CrearHash[string, string]()
	SET_STRING_DATOS := []string{"A", "B", "C", "D", "E", "F", "G", "H", "I", "J"}
	for i := 0; i < 10; i++ {
		SET_STRING_CLAVES[i] = fmt.Sprintf(cadena, i)
		dic.Guardar(SET_STRING_CLAVES[i], SET_STRING_DATOS[i])
	}
	require.EqualValues(t, 10, dic.Cantidad())

	ok := true
	for i := 0; i < 10 && ok; i++ {
		ok = dic.Obtener(SET_STRING_CLAVES[i]) == SET_STRING_DATOS[i]
	}
...
func buscar(clave string, SET_STRING_CLAVES []string) int {
	for i, c := range SET_STRING_CLAVES {
		if c == clave {
			return i
		}
	}
...
func TestIteradorInternoSET_STRING_CLAVES(t *testing.T) {
	t.Log("Valida que todas las SET_STRING_CLAVES sean recorridas (y una única vez) con el iterador interno")
	clave1 := "Gato"
	clave2 := "Perro"
	clave3 := "Vaca"
	SET_STRING_CLAVES := []string{clave1, clave2, clave3}
	dic := TDADiccionario.CrearHash[string, *int]()
	dic.Guardar(SET_STRING_CLAVES[0], nil)
	dic.Guardar(SET_STRING_CLAVES[1], nil)
	dic.Guardar(SET_STRING_CLAVES[2], nil)
	...
	require.NotEqualValues(t, -1, buscar(cs[0], SET_STRING_CLAVES))
	require.NotEqualValues(t, -1, buscar(cs[1], SET_STRING_CLAVES))
	require.NotEqualValues(t, -1, buscar(cs[2], SET_STRING_CLAVES))
	require.NotEqualValues(t, cs[0], cs[1])
	require.NotEqualValues(t, cs[0], cs[2])
	require.NotEqualValues(t, cs[2], cs[1])
}

func TestIteradorInternoSET_STRING_DATOS(t *testing.T) {
...
func TestIteradorInternoSET_STRING_DATOSConBorrados(t *testing.T) {
...
func ejecutarPruebaVolumen(b *testing.B, n int) {
	...
	SET_STRING_CLAVES := make([]string, n)
	SET_STRING_DATOS := make([]int, n)

	/* Inserta 'n' parejas en el hash */
	for i := 0; i < n; i++ {
		SET_STRING_DATOS[i] = i
		SET_STRING_CLAVES[i] = fmt.Sprintf("%08d", i)
		dic.Guardar(SET_STRING_CLAVES[i], SET_STRING_DATOS[i])
	}

	require.EqualValues(b, n, dic.Cantidad(), "La cantidad de elementos es incorrecta")

	/* Verifica que devuelva los SET_STRING_DATOS correctos */
	ok := true
	for i := 0; i < n; i++ {
		ok = dic.Pertenece(SET_STRING_CLAVES[i])
		if !ok {
			break
		}
		ok = dic.Obtener(SET_STRING_CLAVES[i]) == SET_STRING_DATOS[i]
	...
	/* Verifica que borre y devuelva los SET_STRING_DATOS correctos */
	for i := 0; i < n; i++ {
		ok = dic.Borrar(SET_STRING_CLAVES[i]) == SET_STRING_DATOS[i]
		if !ok {
			break
		}
		ok = !dic.Pertenece(SET_STRING_CLAVES[i])
...
func BenchmarkDiccionario(b *testing.B) {
	b.Log("Prueba de stress del Diccionario. Prueba guardando distinta cantidad de elementos (muy grandes), " +
		...
		"sea la adecuada. Luego validamos que podemos obtener y ver si pertenece cada una de las SET_STRING_CLAVES geeneradas, " +
...
func TestDiccionarioIterar(t *testing.T) {
	t.Log("Guardamos 3 SET_STRING_DATOS en un Diccionario, e iteramos validando que las SET_STRING_CLAVES sean todas diferentes " +
		"pero pertenecientes al diccionario. Además los SET_STRING_DATOS de VerActual y Siguiente van siendo correctos entre sí")
	clave1 := "Gato"
	clave2 := "Perro"
	clave3 := "Vaca"
	valor1 := "miau"
	...
	require.NotEqualValues(t, -1, buscar(primero, SET_STRING_CLAVES))

	iter.Siguiente()
	segundo, segundo_valor := iter.VerActual()
	require.NotEqualValues(t, -1, buscar(segundo, SET_STRING_CLAVES))
	require.EqualValues(t, SET_STRING_DATOS[buscar(segundo, SET_STRING_CLAVES)], segundo_valor)
	...
	require.NotEqualValues(t, -1, buscar(tercero, SET_STRING_CLAVES))
...
func TestIteradorNoLlegaAlFinal(t *testing.T) {
	...
	dic := TDADiccionario.CrearHash[string, string]()
	SET_STRING_CLAVES := []string{"A", "B", "C"}
	dic.Guardar(SET_STRING_CLAVES[0], "")
	dic.Guardar(SET_STRING_CLAVES[1], "")
	dic.Guardar(SET_STRING_CLAVES[2], "")
...
func ejecutarPruebasVolumenIterador(b *testing.B, n int) {
	...
	// Prueba de iteración sobre las SET_STRING_CLAVES almacenadas.
	iter := dic.Iterador()
